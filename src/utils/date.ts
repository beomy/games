export default class DateUtil {
  static MsToStr (ms): string {
    ...
    const msMin = Math.floor((ms - (msYear * year) - (msDay * day) - (msHour * hour)) / min)
    const msSec = Math.floor((ms - (msYear * year) - (msDay * day) - (msHour * hour) - (msMin * min)) / sec)

    let str = ''
    if (msYear) {
      str = `${msYear}년 ${msDay || 0}일 ${msHour || 0}시간 ${msMin || 0}분 ${msSec || 0}초`
    } else if (msDay) {
